...
class FmbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('fmb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fmb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fmb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fmb (date, title, link, content)"
